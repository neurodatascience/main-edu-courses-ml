...
\label{sec:orgad92e6b}
\subsection{Recap of part 1}
\label{sec:org5bcc021}
\begin{frame}[label={sec:org97355ea}]{Recap of part 1}
...
\begin{frame}[label={sec:orgb383181}]{Recap of part 1}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgf3f0242}]{Notation \& vocabulary}
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org1dbd7ab}]{How to set parameters: Empirical Risk Minimization}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org9241587}]{How to set parameters: Empirical Risk Minimization}
...
\begin{frame}[label={sec:org9066067}]{Evaluating a model}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org078e091}]{Training error is a biased estimator of the risk}
\begin{itemize}
\item 30 different animals made prognostics about match results in the 2018 World Cup
\end{itemize}
\begin{center}
\includegraphics[height=.65 \textheight]{figures/generated/select_evaluate/select_evaluate_1.pdf}
\end{center}
\end{frame}
\begin{frame}[label={sec:orgc2087a5}]{Training error is a biased estimator of the risk}
\begin{itemize}
\item 30 different animals made prognostics about match results in the 2018 world cup
\item We \alert{selected the best predictor} (highest average accuracy)
\end{itemize}
\begin{center}
\includegraphics[height=.65 \textheight]{figures/generated/select_evaluate/select_evaluate_2.pdf}
\end{center}
\end{frame}
\begin{frame}[label={sec:orgee31231}]{Training error is a biased estimator of the risk}
The same animal is invited to make predictions about the 2022 World Cup.
Is it more likely to perform on average:
\begin{enumerate}
...
\begin{frame}[label={sec:orgd0484d5}]{Training error is a biased estimator of the risk}
The selected animal is more likely to perform \alert{worse} on average than in 2018.
\begin{center}
\includegraphics[height=.65 \textheight]{figures/generated/select_evaluate/select_evaluate_3.pdf}
\end{center}
\end{frame}
\begin{frame}[label={sec:org0d756a9}]{Training error is a biased estimator of the risk}
The selected animal is more likely to perform \alert{worse} on average than in 2018.
\begin{center}
...
\end{frame}
\begin{frame}[label={sec:org7774224}]{Training error is a biased estimator of the risk}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgbe2384d}]{Training error is a biased estimator of the risk}
\begin{itemize}
...
\begin{frame}[label={sec:org56f5d32}]{Estimating prediction performance}
...
\label{sec:orgfad760a}
\begin{frame}[label={sec:orgf9ccfd1},fragile]{scikit-learn "estimator API": \texttt{fit; predict}}
 \begin{minted}[]{python}
estimator = Ridge()
...
\end{frame}
\begin{frame}[label={sec:org88da5c7},fragile]{Evaluating performance with \texttt{sklearn.metrics}}
 \begin{minted}[]{python}
estimator = Ridge()
estimator.fit(X_train, y_train)
predictions = estimator.predict(X_test)
...
\begin{frame}[label={sec:org5d3f949},fragile]{Some possible metrics for regression}
...
\subsection{cv}
\label{sec:org135972c}
\begin{frame}[label={sec:orge61a707},fragile]{Cross-validation}
 \begin{center}
...
\label{sec:org8f82396}
\subsection{nested cv}
\label{sec:orgd968cc8}
\begin{frame}[label={sec:org4bc6845}]{Need for regularization}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:orgdc4acd2}]{Regularization}
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org5fa0bb6}]{}
\begin{columns}
...
\begin{frame}[label={sec:orgfea843e}]{Setting hyperparameters}
...
\end{frame}
\begin{frame}[label={sec:org5514a6f}]{Nested cross-validation}
When you hear "best", "maximum", "select", \ldots{} think "bias"
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgcc2df95}]{One split}
\begin{center}
\includegraphics[width=.9\linewidth]{figures/generated/train_eval_test/datasets.pdf}
\end{center}
\end{frame}
\begin{frame}[label={sec:org67f844b},fragile]{Nested cross-validation}
 \begin{center}
...
\begin{frame}[label={sec:orgf8f30a0},fragile]{Nested cross-validation with scikit-learn}
 \begin{itemize}
...
\end{frame}
\begin{frame}[label={sec:org279395a},fragile]{Implementing nested CV}
...
\label{sec:org4310d19}
\subsection{Intro}
\label{sec:org03a0a6e}
\begin{frame}[label={sec:org21c2d3f}]{Dimensionality reduction}
Linear regression: projection on the column space of \(X\)
\vspace{10pt}
\begin{structureenv} %% top
\begin{columns}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org231530f}]{Dimensionality reduction}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orga72514c},fragile]{Simulated data for linear regression}
 \begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgc7481bc}]{Model complexity: overfitting}
\begin{itemize}
...
\label{sec:org25d7162}
\begin{frame}[label={sec:org5b26cf3}]{Univariate feature selection}
\begin{itemize}
...
\begin{frame}[label={sec:org83c5607}]{Original regression problem}
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
\hat{\y} = \X \, \hat{\bbeta}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org1661e7d}]{After univariate feature selection}
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
\hat{\y} = \X \, \hat{\bbeta}
...
\begin{frame}[label={sec:org7541661}]{After univariate feature selection}
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
\hat{\y} = \X \, \hat{\bbeta}
...
\begin{frame}[label={sec:orgb7537f8}]{Univariate feature selection}
...
\label{sec:org1d56bc8}
\begin{frame}[label={sec:org938c15d}]{Dataset transformations}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgef6fb2b},fragile]{scikit-learn "transformer API": \texttt{fit; transform}}
 \begin{minted}[]{python}
...
\begin{frame}[label={sec:org7d28ba2},fragile]{\texttt{feature\_selection.SelectKBest}}
...
\begin{frame}[label={sec:org6918c60},fragile]{Fit the transformer only on train data!}
 \begin{minted}[]{python}
transformer = SelectKBest()
transformed_train = transformer.fit_transform(
    X_train, y_train)
...
\begin{frame}[label={sec:org2a89d7b},fragile]{Pipelines}
...
\label{sec:org2eea63d}
\begin{frame}[label={sec:org4838378}]{Linear decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org90320ff}]{Linear regression: projection on the column space of \(X\)}
\begin{structureenv} %% top
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org0f30fa4}]{Linear decomposition: low-rank approximation of \(\X\)}
Minimize
\begin{equation}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgce73a27}]{Linear regression after dimensionality reduction}
\begin{equation}
\hat{\y} = \W \, \hat{\bbeta}
\end{equation}
\begin{center}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org8875d98}]{Prediction for a new data point \(\x \in \R^{p}\)}
\begin{itemize}
...
\x \in \R^p \rightarrow \text{projection} \rightarrow \mathbold{w} \in \R^k \rightarrow \langle \cdot \, , \, \hat{\bbeta}\rangle \rightarrow \hat{y} \in \R
    \end{equation}
\end{frame}
\begin{frame}[label={sec:orge4d7c47}]{Principal Component Analysis}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orged29bb5}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:org673ca18}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:org938227b}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:orgad820be}]{Other decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org75e8cf0}]{Ridge regression and PCA}
\begin{itemize}
...
\label{sec:orgb43f60d}
\begin{frame}[label={sec:orgb752265}]{(Cross-)validation experiments are simulations}
...
\end{frame}
\begin{frame}[label={sec:orgb2bcfba}]{(Cross-)validation experiments are simulations}
The validation experiments must simulate what will happen when deploying the trained model in production -- when starting to use it in real life.
...
\end{example}
\end{frame}
\begin{frame}[label={sec:orgad53fd0}]{Split choice example: time series}
...
\end{frame}
\begin{frame}[label={sec:org88283ce}]{Remember that CV training sets overlap}
\begin{center}
...
\begin{frame}[label={sec:orgbe38811},fragile]{Some pitfalls with cross-validation}
 \small
