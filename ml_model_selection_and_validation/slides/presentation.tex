...
\label{sec:org0572e78}
\subsection{Instructions}
\label{sec:org3b10259}
\begin{frame}[label={sec:orgd040e2a},fragile]{Setup reminder}
...
\vspace{-20pt}
\begin{itemize}
\item repository: \href{https://github.com/neurodatascience/main-edu-courses-ml}{\texttt{github.com/neurodatascience/main-edu-courses-ml}}
\item \href{https://drive.google.com/drive/folders/1fI8RPwqSsxIHeC0nGDbElaGTOK2q3qIy}{notebooks in google drive}
\end{itemize}
\end{frame}
\subsection{Recap of part 1}
\label{sec:org27a5350}
\begin{frame}[label={sec:orgc0665a9}]{Recap of part 1}
...
\begin{frame}[label={sec:orgcb3b53e}]{Recap of part 1}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org52ff6a6}]{Notation \& vocabulary}
...
\end{example}
\end{frame}
\begin{frame}[label={sec:orgdd15be1}]{How to set parameters: Empirical Risk Minimization}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org15bcf23}]{How to set parameters: Empirical Risk Minimization}
...
\begin{frame}[label={sec:orgace7204}]{Evaluating a model}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org48ca35c}]{Training error is a biased estimator of the risk}
\begin{itemize}
\item 30 different animals made predictions about match results in the 2018 World Cup
\end{itemize}
\begin{center}
\includegraphics[height=.65 \textheight]{figures/generated/select_evaluate/select_evaluate_1.pdf}
\end{center}
\end{frame}
\begin{frame}[label={sec:orgbaf280e}]{Training error is a biased estimator of the risk}
\begin{itemize}
\item 30 different animals made predictions about match results in the 2018 world cup
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org9bb4879}]{Training error is a biased estimator of the risk}
The same animal is invited to make predictions about the 2022 World Cup.
Is it more likely to perform:
\begin{enumerate}
\item Better than in 2018?
\item Worse than in 2018?
\item The same?
\end{enumerate}
\begin{center}
...
\begin{frame}[label={sec:org07278a7}]{Training error is a biased estimator of the risk}
The selected animal is more likely to perform \alert{worse} than in 2018.
\begin{center}
\includegraphics[height=.65 \textheight]{figures/generated/select_evaluate/select_evaluate_3.pdf}
\end{center}
\end{frame}
\begin{frame}[label={sec:orgf23abf5}]{Training error is a biased estimator of the risk}
The selected animal is more likely to perform \alert{worse} than in 2018.
\begin{center}
\includegraphics[height=.65 \textheight]{figures/generated/select_evaluate/select_evaluate_3b.pdf}
\end{center}
Its 2018 performance is a \alert{biased} estimator of its expected performance in future World Cups.
\end{frame}
\begin{frame}[label={sec:org881d19a}]{Training error is a biased estimator of the risk}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org5d9bb9a}]{Training error is a biased estimator of the risk}
\begin{itemize}
...
\begin{frame}[label={sec:org6f74f6c}]{Estimating prediction performance}
...
\label{sec:org60c7687}
\begin{frame}[label={sec:org3e387f6},fragile]{scikit-learn "estimator API": \texttt{fit; predict}}
 \begin{minted}[]{python}
estimator = Ridge()
...
\end{frame}
\begin{frame}[label={sec:org5858697},fragile]{Evaluating performance with \texttt{sklearn.metrics}}
 \begin{minted}[]{python}
estimator = Ridge()
estimator.fit(X_train, y_train)
predictions = estimator.predict(X_test)
...
\begin{frame}[label={sec:orga2571bb},fragile]{Some possible metrics for regression}
...
\subsection{cv}
\label{sec:org4e541e8}
\begin{frame}[label={sec:org82bb302},fragile]{Cross-validation}
 \begin{center}
...
\label{sec:orga8d2828}
\subsection{nested cv}
\label{sec:orgce3e60b}
\begin{frame}[label={sec:orgb99e295}]{Need for regularization}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org6b08a79}]{Regularization}
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org5ed467f}]{}
\begin{columns}
...
\begin{frame}[label={sec:orgc197677}]{Setting hyperparameters}
...
\end{frame}
\begin{frame}[label={sec:orgb40e2a5}]{Nested cross-validation}
When you hear "best", "maximum", "select", \ldots{} think "bias"
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org4a61b21}]{One split}
\begin{center}
\includegraphics[width=.9\linewidth]{figures/generated/train_eval_test/datasets.pdf}
\end{center}
\end{frame}
\begin{frame}[label={sec:orga1791c1},fragile]{Nested cross-validation}
 \begin{center}
...
\begin{frame}[label={sec:org74d43e0},fragile]{Nested cross-validation with scikit-learn}
 \begin{itemize}
...
\end{frame}
\begin{frame}[label={sec:orged1dd13},fragile]{Implementing nested CV}
...
\label{sec:org67b3e5a}
\subsection{Intro}
\label{sec:org3ce295b}
\begin{frame}[label={sec:org5e1d21f}]{Dimensionality reduction}
Linear regression: projection on the column space of \(X\)
\vspace{10pt}
\begin{structureenv} %% top
\begin{columns}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org3d05588}]{Dimensionality reduction}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orge52f9e4},fragile]{Simulated data for linear regression}
 \begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org5856a97}]{Model complexity: overfitting}
\begin{itemize}
...
\label{sec:org8534a91}
\begin{frame}[label={sec:orgbad2284}]{Univariate feature selection}
\begin{itemize}
...
\begin{frame}[label={sec:orgfb71dbd}]{Original regression problem}
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
\hat{\y} = \X \, \hat{\bbeta}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org73fb8b2}]{After univariate feature selection}
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
\hat{\y} = \X \, \hat{\bbeta}
...
\begin{frame}[label={sec:orgb823d6b}]{After univariate feature selection}
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
\hat{\y} = \X \, \hat{\bbeta}
...
\begin{frame}[label={sec:org4770c84}]{Univariate feature selection}
...
\label{sec:orgcd646f0}
\begin{frame}[label={sec:org2f1b6af}]{Dataset transformations}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org6531b71},fragile]{scikit-learn "transformer API": \texttt{fit; transform}}
 \begin{minted}[]{python}
...
\begin{frame}[label={sec:orga5c1775},fragile]{\texttt{feature\_selection.SelectKBest}}
...
\begin{frame}[label={sec:orgf8191cf},fragile]{Fit the transformer only on train data!}
 \begin{minted}[]{python}
transformer = SelectKBest()
transformed_train = transformer.fit_transform(
    X_train, y_train)
...
\begin{frame}[label={sec:orge2443d5},fragile]{Pipelines}
...
\label{sec:orgd8c8d6e}
\begin{frame}[label={sec:orgc5f00bc}]{Linear decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org0355bea}]{Linear regression: projection on the column space of \(X\)}
\begin{structureenv} %% top
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org8d6ef70}]{Linear decomposition: low-rank approximation of \(\X\)}
Minimize
\begin{equation}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org4d9e2d9}]{Linear regression after dimensionality reduction}
\begin{equation}
\hat{\y} = \W \, \hat{\bbeta}
\end{equation}
\begin{center}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org35cdea2}]{Prediction for a new data point \(\x \in \R^{p}\)}
\begin{itemize}
...
\x \in \R^p \rightarrow \text{projection} \rightarrow \mathbold{w} \in \R^k \rightarrow \langle \cdot \, , \, \hat{\bbeta}\rangle \rightarrow \hat{y} \in \R
    \end{equation}
\end{frame}
\begin{frame}[label={sec:org0d74fb1}]{Principal Component Analysis}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgbcdc410}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:orgdf9ec89}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:org07e689d}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:orgbbe7d1f}]{Other decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org77d80e4}]{Ridge regression and PCA}
\begin{itemize}
...
\label{sec:org34b1eb1}
\begin{frame}[label={sec:org0a8633d}]{(Cross-)validation experiments are simulations}
...
\end{frame}
\begin{frame}[label={sec:org31274d9}]{(Cross-)validation experiments are simulations}
The validation experiments must simulate what will happen when deploying the trained model in production -- when starting to use it in real life.
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org80687f7}]{Split choice example: time series}
...
\end{frame}
\begin{frame}[label={sec:org8e06d59}]{Remember that CV training sets overlap}
\begin{center}
...
\begin{frame}[label={sec:orgb4b38bf},fragile]{Some pitfalls with cross-validation}
 \small
