...
\label{sec:org724665a}
\subsection{Recap of part 1}
\label{sec:orgfa9db70}
\begin{frame}[label={sec:org775220c}]{Recap of part 1}
...
\begin{frame}[label={sec:org760728c}]{Recap of part 1}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org7f14ce2}]{Notation \& vocabulary}
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org3e9b3d0}]{How to set parameters: Empirical Risk Minimization}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org35aecc0}]{How to set parameters: Empirical Risk Minimization}
...
\begin{frame}[label={sec:org39163b3}]{Evaluating a model}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:orgcc5ba72}]{Training error is a biased estimator of the risk}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org5428f83}]{Training error is a biased estimator of the risk}
\begin{itemize}
\item 30 different animals made prognostics about match results in the 2018 world cup
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org1600f49}]{Training error is a biased estimator of the risk}
...
\begin{frame}[label={sec:orgab5700d}]{Training error is a biased estimator of the risk}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org166fe4a}]{Training error is a biased estimator of the risk}
The selected animal is more likely to perform \alert{worse} on average than in 2018.
\begin{center}
...
\end{frame}
\begin{frame}[label={sec:org654abe5}]{Training error is a biased estimator of the risk}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgde673f7}]{Training error is a biased estimator of the risk}
\begin{itemize}
...
\begin{frame}[label={sec:org1912620}]{Estimating prediction performance}
...
\label{sec:org7570806}
\begin{frame}[label={sec:org5889fe7},fragile]{scikit-learn "estimator API": \texttt{fit; predict}}
 \begin{minted}[]{python}
estimator = Ridge()
...
\end{frame}
\begin{frame}[label={sec:org6ead928},fragile]{Evaluating performance with \texttt{sklearn.metrics}}
 \begin{minted}[]{python}
estimator = Ridge()
estimator.fit(X_train, y_train)
predictions = estimator.predict(X_test)
...
\begin{frame}[label={sec:orgae40715},fragile]{Some possible metrics for regression}
...
\subsection{cv}
\label{sec:org2240931}
\begin{frame}[label={sec:org4e50e63},fragile]{Cross-validation}
 \begin{center}
...
\label{sec:org7739aa8}
\subsection{nested cv}
\label{sec:orgcf3e905}
\begin{frame}[label={sec:org6eab31a}]{Need for regularization}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org8f93e7c}]{Regularization}
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org68367db}]{}
\begin{columns}
...
\begin{frame}[label={sec:orge1eb35e}]{Setting hyperparameters}
...
\end{frame}
\begin{frame}[label={sec:orgca7e90c}]{Nested cross-validation}
When you hear "best", "maximum", "select", \ldots{} think "bias"
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org2627955}]{One split}
\begin{center}
\includegraphics[width=.9\linewidth]{figures/generated/train_eval_test/datasets.pdf}
\end{center}
\end{frame}
\begin{frame}[label={sec:org8f6127a},fragile]{Nested cross-validation}
 \begin{center}
...
\begin{frame}[label={sec:org1391907},fragile]{Nested cross-validation with scikit-learn}
 \begin{itemize}
...
\end{frame}
\begin{frame}[label={sec:orgbf29674},fragile]{Implementing nested CV}
...
\label{sec:org3581b97}
\subsection{Intro}
\label{sec:orga14a3fd}
\begin{frame}[label={sec:org3346161}]{Dimensionality reduction}
Linear regression: projection on the column space of \(X\)
\vspace{10pt}
\begin{structureenv} %% top
\begin{columns}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:orga63b6cf}]{Dimensionality reduction}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org2af9201},fragile]{Simulated data for linear regression}
 \begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgd18989a}]{Model complexity: overfitting}
\begin{itemize}
...
\label{sec:orgf6f6f72}
\begin{frame}[label={sec:org55f407a}]{Univariate feature selection}
\begin{itemize}
...
\begin{frame}[label={sec:orgf8b277b}]{Original regression problem}
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
\hat{\y} = \X \, \hat{\bbeta}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org4e64a41}]{After univariate feature selection}
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
\hat{\y} = \X \, \hat{\bbeta}
...
\begin{frame}[label={sec:orgaca40c0}]{After univariate feature selection}
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
\hat{\y} = \X \, \hat{\bbeta}
...
\begin{frame}[label={sec:org475cae1}]{Univariate feature selection}
...
\label{sec:orgdc06967}
\begin{frame}[label={sec:org46f47f5}]{Dataset transformations}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org3e3d5a8},fragile]{scikit-learn "transformer API": \texttt{fit; transform}}
 \begin{minted}[]{python}
...
\begin{frame}[label={sec:org343ce81},fragile]{\texttt{feature\_selection.SelectKBest}}
...
\begin{frame}[label={sec:orge353f3d},fragile]{Fit the transformer only on train data!}
 \begin{minted}[]{python}
transformer = SelectKBest()
transformed_train = transformer.fit_transform(
    X_train, y_train)
...
\begin{frame}[label={sec:orgd45426f},fragile]{Pipelines}
...
\label{sec:org100b530}
\begin{frame}[label={sec:org8da6907}]{Linear decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgb32cdbf}]{Linear regression: projection on the column space of \(X\)}
\begin{structureenv} %% top
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org3de24c0}]{Linear decomposition: low-rank approximation of \(\X\)}
Minimize
\begin{equation}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgdf6d478}]{Linear regression after dimensionality reduction}
\begin{equation}
\hat{\y} = \W \, \hat{\bbeta}
\end{equation}
\begin{center}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgba8bb03}]{Prediction for a new data point \(\x \in \R^{p}\)}
\begin{itemize}
...
\x \in \R^p \rightarrow \text{projection} \rightarrow \mathbold{w} \in \R^k \rightarrow \langle \cdot \, , \, \hat{\bbeta}\rangle \rightarrow \hat{y} \in \R
    \end{equation}
\end{frame}
\begin{frame}[label={sec:org427d789}]{Principal Component Analysis}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org4447e17}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:orgc907421}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:org91f967a}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:org2377a97}]{Other decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org25522dd}]{Ridge regression and PCA}
\begin{itemize}
...
\label{sec:org422522a}
\begin{frame}[label={sec:org1b42a27}]{(Cross-)validation experiments are simulations}
...
\end{frame}
\begin{frame}[label={sec:org0b3e242}]{(Cross-)validation experiments are simulations}
The validation experiments must simulate what will happen when deploying the trained model in production -- when starting to use it in real life.
...
\end{example}
\end{frame}
\begin{frame}[label={sec:orgd735419}]{Split choice example: time series}
...
\end{frame}
\begin{frame}[label={sec:org1866149}]{Remember that CV training sets overlap}
\begin{center}
...
\begin{frame}[label={sec:orgc913764},fragile]{Some pitfalls with cross-validation}
 \small
