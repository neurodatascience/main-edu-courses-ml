...
\maketitle
\section{Intro}
\label{sec:org7352da0}
\subsection{Recap of part 1}
\label{sec:org23ee727}
\begin{frame}[label={sec:org77c16bc}]{Recap of part 1}
...
\begin{frame}[label={sec:org24aab4c}]{Recap of part 1}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org20b2f09}]{Notation \& vocabulary}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org4a462ab}]{How to estimate parameters: Empirical Risk Minimization}
...
\end{frame}
\begin{frame}[label={sec:org01c71ee}]{Need for fresh test data}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org779e73d}]{Estimating prediction performance}
When you hear "best", "maximum", "select", \ldots{} think "bias"
...
\label{sec:org1d2d15a}
\begin{frame}[label={sec:org286f172},fragile]{scikit-learn "estimator API": \texttt{fit; predict}}
 \begin{verbatim}
estimator = Ridge()
...
\begin{frame}[label={sec:org74ea98c},fragile]{Evaluating performance with \texttt{sklearn.metrics}}
 \begin{verbatim}
estimator = Ridge()
estimator.fit(X_train, y_train)
predictions = estimator.predict(X_test)
...
\label{sec:orgbb72e20}

\subsection{cv}
\label{sec:org25ea84f}
\begin{frame}[label={sec:org0c9da09},fragile]{Cross-validation}
 \begin{center}
...
\label{sec:org5410742}

\subsection{nested cv}
\label{sec:org6d2836d}

\begin{frame}[label={sec:org355d596}]{}
\begin{columns}
...
\begin{frame}[label={sec:org24ca2ed}]{Setting hyperparameters}
\begin{block}{How can we choose the ridge hyperparameter \(\lambda\)?}
\end{block}
Try a few and pick the best one\ldots{}

But measure its performance on separate data!
\end{frame}
\begin{frame}[label={sec:org64a9a8e}]{Nested cross-validation}
When you hear "best", "maximum", "select", \ldots{} think "bias"
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orge7809d4}]{One split}
\begin{center}
...
\begin{frame}[label={sec:orge3e564f},fragile]{Nested cross-validation}
 \begin{center}
...
\begin{frame}[label={sec:orgc54d8f2},fragile]{Implementing nested CV}
 \texttt{ex\_05\_nested\_cross\_validation.py}
\end{frame}
\section{Dimensionality reduction}
\label{sec:org801ab3d}
\subsection{Intro}
\label{sec:orgd7b72f8}
\begin{frame}[label={sec:orgaa5c697}]{Dimensionality reduction}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgec89719}]{Dimensionality reduction}
\begin{equation}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org1cd91b0},fragile]{Simulated data for linear regression}
 \begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org88d4b02}]{Model complexity: overfitting}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgee0a38a}]{Same plot in log scale}
\begin{center}
...
\label{sec:org39185c3}
\begin{frame}[label={sec:orgab9d12e}]{Solution 1: univariate feature selection}
\begin{itemize}
...
\begin{frame}[label={sec:org563b7e6}]{Univariate feature selection}
...
\begin{frame}[label={sec:org1439a74}]{Same plot in log scale}
\begin{center}
...
\label{sec:org34b1036}
\begin{frame}[label={sec:orgf783ae1}]{Dataset transformations}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgc2547ab},fragile]{scikit-learn "transformer API": \texttt{fit; transform}}
 \begin{verbatim}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:orgdcd440d},fragile]{scikit-learn "transformer API": \texttt{fit; transform}}
 \begin{verbatim}
transformer = SelectKBest()
transformed_X = transformer.fit_transform(X_train)
...
\href{https://scikit-learn.org/stable/data\_transforms.html}{scikit-learn "user guide"}
\end{frame}
\begin{frame}[label={sec:org167954d},fragile]{Example: \texttt{feature\_selection.SelectKBest}}
...
\label{sec:org28052d1}
\begin{frame}[label={sec:org19ae98a}]{Solution 2: linear decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgac9bc06}]{Linear regression: projection on the column space of \(X\)}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org49f46dd}]{Linear decomposition: low-rank approximation of \(\X\)}
Minimize
\begin{equation}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org0412a17}]{Linear regression after dimensionality reduction}
\begin{equation}
\hat{\y} = \W \, \hat{\bbeta}
\end{equation}
\begin{center}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org48ad7c6}]{Prediction for a new data point \(\x \in \R^{p}\)}
\begin{itemize}
...
\x \in \R^p \rightarrow \text{projection} \rightarrow \mathbold{w} \in \R^k \rightarrow \langle \cdot \, , \, \hat{\bbeta}\rangle \rightarrow \hat{y} \in \R
    \end{equation}
\end{frame}
\begin{frame}[label={sec:org0a28282}]{Principal Component Analysis}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org1c8b08c}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:org2bc73fa}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:orgc493aa6}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:org6189ce5}]{Other decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgf3ba0eb}]{Ridge regression and PCA}
\begin{itemize}
...
\label{sec:org057d925}
\begin{frame}[label={sec:org519ff9f},fragile]{Some common pitfalls with cross-validation}
...
\begin{frame}[label={sec:orgae0ff30}]{Split choice example: time series}
Which is easier?
\begin{center}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgd949b03}]{Remember that CV training sets overlap}
\begin{center}
...
\label{sec:orgc817c91}
\subsection{FMRI decoding}
\label{sec:org944cc88}
\begin{frame}[label={sec:org1012278}]{Supervised learning with fMRI}
\begin{itemize}
\item Predict in which site / with which scanner a resting-state fMRI sequence was acquired
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org14ed730}]{The prediction pipeline}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org39c229d}]{Implementation: in class}
\end{frame}
\end{document}