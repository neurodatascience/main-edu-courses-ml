...
\label{sec:org386943d}
\subsection{Recap of part 1}
\label{sec:org24b43b3}
\begin{frame}[label={sec:org9093bb6}]{Recap of part 1}
...
\begin{frame}[label={sec:orgcbfbf1f}]{Recap of part 1}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org0a3fc4c}]{Notation \& vocabulary}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org2682948}]{How to estimate parameters: Empirical Risk Minimization}
...
\end{frame}
\begin{frame}[label={sec:org53336fa}]{Need for fresh test data}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orge687d2a}]{Estimating prediction performance}
When you hear "best", "maximum", "select", \ldots{} think "bias"
...
\label{sec:org5ca5594}
\begin{frame}[label={sec:org930e202},fragile]{scikit-learn "estimator API": \texttt{fit; predict}}
 \begin{verbatim}
estimator = Ridge()
...
\begin{frame}[label={sec:org40b5132},fragile]{Evaluating performance with \texttt{sklearn.metrics}}
 \begin{verbatim}
estimator = Ridge()
estimator.fit(X_train, y_train)
predictions = estimator.predict(X_test)
...
\subsection{cv}
\label{sec:org89d780c}
\begin{frame}[label={sec:org3d42eb1},fragile]{Cross-validation}
 \begin{center}
...
\label{sec:org11b5758}
\subsection{nested cv}
\label{sec:org4a2e59d}
\begin{frame}[label={sec:org74dd44a}]{Regularization}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgf3c3128}]{}
\begin{columns}
...
\begin{frame}[label={sec:orgea145c9}]{Setting hyperparameters}
...
\end{frame}
\begin{frame}[label={sec:org447b66a}]{Nested cross-validation}
When you hear "best", "maximum", "select", \ldots{} think "bias"
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org8fbac74}]{One split}
\begin{center}
\includegraphics[width=.9\linewidth]{figures/generated/train_eval_test/datasets.pdf}
\end{center}
\end{frame}
\begin{frame}[label={sec:orgfaef59f},fragile]{Nested cross-validation}
 \begin{center}
...
\begin{frame}[label={sec:orgad554af},fragile]{Implementing nested CV}
 \texttt{ex\_05\_nested\_cross\_validation.py}
\end{frame}
\section{Dimensionality reduction}
\label{sec:org2197992}
\subsection{Intro}
\label{sec:org4a4f901}
\begin{frame}[label={sec:orgdfd45d2}]{Dimensionality reduction}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgd6e3210}]{Dimensionality reduction}
\begin{equation}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org250734e},fragile]{Simulated data for linear regression}
 \begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgfaf7d87}]{Model complexity: overfitting}
\begin{itemize}
...
\label{sec:org39e7850}
\begin{frame}[label={sec:org9ac6973}]{Univariate feature selection}
\begin{itemize}
...
\begin{frame}[label={sec:org62816f2}]{Univariate feature selection}
...
\label{sec:org7d39106}
\begin{frame}[label={sec:org8c33cbe}]{Dataset transformations}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org7394079},fragile]{scikit-learn "transformer API": \texttt{fit; transform}}
 \begin{verbatim}
...
\begin{frame}[label={sec:org91ea4ea},fragile]{\texttt{feature\_selection.SelectKBest}}
...
\begin{frame}[label={sec:org0b72c3d},fragile]{scikit-learn "transformer API": \texttt{fit; transform}}
 \begin{verbatim}
transformer = SelectKBest()
...
\label{sec:org63c218d}
\begin{frame}[label={sec:org926fadb}]{Solution 2: linear decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org6fae60d}]{Linear regression: projection on the column space of \(X\)}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org8c9b9f7}]{Linear decomposition: low-rank approximation of \(\X\)}
Minimize
\begin{equation}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org2589e0e}]{Linear regression after dimensionality reduction}
\begin{equation}
\hat{\y} = \W \, \hat{\bbeta}
\end{equation}
\begin{center}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org0ff0c9f}]{Prediction for a new data point \(\x \in \R^{p}\)}
\begin{itemize}
...
\x \in \R^p \rightarrow \text{projection} \rightarrow \mathbold{w} \in \R^k \rightarrow \langle \cdot \, , \, \hat{\bbeta}\rangle \rightarrow \hat{y} \in \R
    \end{equation}
\end{frame}
\begin{frame}[label={sec:orgdc978ad}]{Principal Component Analysis}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org1e32758}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:orgdbd5c9a}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:org29f56b2}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:orgdf189fc}]{Other decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgf995833}]{Ridge regression and PCA}
\begin{itemize}
...
\end{frame}
\section{Conclusion}
\label{sec:org19603a5}
\begin{frame}[label={sec:orgad2c429},fragile]{Some pitfalls with cross-validation}
...
\begin{frame}[label={sec:org3ad1ddc}]{Split choice example: time series}
Which is easier?
\begin{center}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org9608d22}]{Remember that CV training sets overlap}
\begin{center}
