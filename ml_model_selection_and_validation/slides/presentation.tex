...
\label{sec:org2c28a01}
\subsection{Recap of part 1}
\label{sec:orgfaa7261}
\begin{frame}[label={sec:org2e83bdc}]{Recap of part 1}
...
\begin{frame}[label={sec:org558cf61}]{Recap of part 1}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgd5dad1b}]{Notation \& vocabulary}
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org94cf000}]{How to set parameters: Empirical Risk Minimization}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org46660dc}]{How to set parameters: Empirical Risk Minimization}
...
\begin{frame}[label={sec:orgd905f29}]{Evaluating a model}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org70677fc}]{Training error is a biased estimator of the risk}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgbe73ca3}]{Training error is a biased estimator of the risk}
\begin{itemize}
\item 30 different animals made prognostics about match results in the 2018 world cup
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgc7c4436}]{Training error is a biased estimator of the risk}
...
\begin{frame}[label={sec:org4c9b60a}]{Training error is a biased estimator of the risk}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgef35e64}]{Training error is a biased estimator of the risk}
The selected animal is more likely to perform \alert{worse} on average than in 2018.
\begin{center}
...
\end{frame}
\begin{frame}[label={sec:org41f323c}]{Training error is a biased estimator of the risk}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgd317d17}]{Training error is a biased estimator of the risk}
\begin{itemize}
...
\begin{frame}[label={sec:orge60f2ac}]{Estimating prediction performance}
...
\label{sec:org84f07f3}
\begin{frame}[label={sec:org3b97c2d},fragile]{scikit-learn "estimator API": \texttt{fit; predict}}
 \begin{minted}[]{python}
estimator = Ridge()
...
\end{frame}
\begin{frame}[label={sec:orged99296},fragile]{Evaluating performance with \texttt{sklearn.metrics}}
 \begin{minted}[]{python}
estimator = Ridge()
estimator.fit(X_train, y_train)
predictions = estimator.predict(X_test)
...
\begin{frame}[label={sec:orgdc894cc},fragile]{Some possible metrics for regression}
...
\subsection{cv}
\label{sec:orgcb5ce8d}
\begin{frame}[label={sec:orgff88d89},fragile]{Cross-validation}
 \begin{center}
...
\label{sec:org0110b5c}
\subsection{nested cv}
\label{sec:org8a22024}
\begin{frame}[label={sec:org9bbd8f5}]{Need for regularization}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org06f1f49}]{Regularization}
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org288be32}]{}
\begin{columns}
...
\begin{frame}[label={sec:org712cbd3}]{Setting hyperparameters}
...
\end{frame}
\begin{frame}[label={sec:org475fd38}]{Nested cross-validation}
When you hear "best", "maximum", "select", \ldots{} think "bias"
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org90d6b39}]{One split}
\begin{center}
\includegraphics[width=.9\linewidth]{figures/generated/train_eval_test/datasets.pdf}
\end{center}
\end{frame}
\begin{frame}[label={sec:org4266e09},fragile]{Nested cross-validation}
 \begin{center}
...
\begin{frame}[label={sec:org28b31e7},fragile]{Nested cross-validation with scikit-learn}
 \begin{itemize}
...
\end{frame}
\begin{frame}[label={sec:org28adfd1},fragile]{Implementing nested CV}
...
\label{sec:org74ff21f}
\subsection{Intro}
\label{sec:org03969a3}
\begin{frame}[label={sec:orgfe8b6af}]{Dimensionality reduction}
Linear regression: projection on the column space of \(X\)
\vspace{10pt}
\begin{structureenv} %% top
\begin{columns}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org27f8d59}]{Dimensionality reduction}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org29dc85a},fragile]{Simulated data for linear regression}
 \begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orge23ccb2}]{Model complexity: overfitting}
\begin{itemize}
...
\label{sec:orgeb54c92}
\begin{frame}[label={sec:org1a1d43c}]{Univariate feature selection}
\begin{itemize}
...
\begin{frame}[label={sec:org4699db3}]{Original regression problem}
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
\hat{\y} = \X \, \hat{\bbeta}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org27cac00}]{After univariate feature selection}
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
\hat{\y} = \X \, \hat{\bbeta}
...
\begin{frame}[label={sec:org9b2ed0f}]{After univariate feature selection}
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
\hat{\y} = \X \, \hat{\bbeta}
...
\begin{frame}[label={sec:org5597ced}]{Univariate feature selection}
...
\label{sec:org237a15a}
\begin{frame}[label={sec:org15746dc}]{Dataset transformations}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgdc9338e},fragile]{scikit-learn "transformer API": \texttt{fit; transform}}
 \begin{minted}[]{python}
...
\begin{frame}[label={sec:org9140f45},fragile]{\texttt{feature\_selection.SelectKBest}}
...
\begin{frame}[label={sec:orgfc05452},fragile]{Fit the transformer only on train data!}
 \begin{minted}[]{python}
transformer = SelectKBest()
transformed_train = transformer.fit_transform(
    X_train, y_train)
...
\begin{frame}[label={sec:orge2d98f5},fragile]{Pipelines}
...
\label{sec:org279499e}
\begin{frame}[label={sec:orgf5a7951}]{Linear decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org52fc0d1}]{Linear regression: projection on the column space of \(X\)}
\begin{structureenv} %% top
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org82cf9b1}]{Linear decomposition: low-rank approximation of \(\X\)}
Minimize
\begin{equation}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgec110e9}]{Linear regression after dimensionality reduction}
\begin{equation}
\hat{\y} = \W \, \hat{\bbeta}
\end{equation}
\begin{center}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgb54c9e9}]{Prediction for a new data point \(\x \in \R^{p}\)}
\begin{itemize}
...
\x \in \R^p \rightarrow \text{projection} \rightarrow \mathbold{w} \in \R^k \rightarrow \langle \cdot \, , \, \hat{\bbeta}\rangle \rightarrow \hat{y} \in \R
    \end{equation}
\end{frame}
\begin{frame}[label={sec:org8a71f12}]{Principal Component Analysis}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org67db83b}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:orgf028ccd}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:org2c81034}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:orga7d6722}]{Other decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orge4c4fcf}]{Ridge regression and PCA}
\begin{itemize}
...
\label{sec:org561056f}
\begin{frame}[label={sec:org021f005}]{(Cross-)validation experiments are simulations}
...
\end{frame}
\begin{frame}[label={sec:orgb62a1c7}]{(Cross-)validation experiments are simulations}
The validation experiments must simulate what will happen when deploying the trained model in production -- when starting to use it in real life.
...
\end{example}
\end{frame}
\begin{frame}[label={sec:orgb9928d9}]{Split choice example: time series}
...
\end{frame}
\begin{frame}[label={sec:org7d3322f}]{Remember that CV training sets overlap}
\begin{center}
...
\begin{frame}[label={sec:orgecdac3f},fragile]{Some pitfalls with cross-validation}
 \small
