...
\label{sec:org98668b3}
\subsection{Recap of part 1}
\label{sec:orge50122f}
\begin{frame}[label={sec:orgc5a3791}]{Recap of part 1}
...
\begin{frame}[label={sec:org9ba59df}]{Recap of part 1}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orge846090}]{Notation \& vocabulary}
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org6bc3284}]{How to set parameters: Empirical Risk Minimization}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org088bb50}]{How to set parameters: Empirical Risk Minimization}
...
\begin{frame}[label={sec:org579d154}]{Evaluating a model}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org4c8e631}]{Training error is a biased estimator of the risk}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgeb9b036}]{Training error is a biased estimator of the risk}
\begin{itemize}
\item 30 different animals made predictions about match results in the 2018 world cup
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org0ef955e}]{Training error is a biased estimator of the risk}
...
\begin{frame}[label={sec:org8c73060}]{Training error is a biased estimator of the risk}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org9cdf1ab}]{Training error is a biased estimator of the risk}
The selected animal is more likely to perform \alert{worse} than in 2018.
\begin{center}
...
\end{frame}
\begin{frame}[label={sec:org74cfc23}]{Training error is a biased estimator of the risk}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgde506cf}]{Training error is a biased estimator of the risk}
\begin{itemize}
...
\begin{frame}[label={sec:org2a49894}]{Estimating prediction performance}
...
\label{sec:orgd91f5a6}
\begin{frame}[label={sec:org31e54ec},fragile]{scikit-learn "estimator API": \texttt{fit; predict}}
 \begin{minted}[]{python}
estimator = Ridge()
...
\end{frame}
\begin{frame}[label={sec:orgcd9d70d},fragile]{Evaluating performance with \texttt{sklearn.metrics}}
 \begin{minted}[]{python}
estimator = Ridge()
estimator.fit(X_train, y_train)
predictions = estimator.predict(X_test)
...
\begin{frame}[label={sec:orgea57e7b},fragile]{Some possible metrics for regression}
...
\subsection{cv}
\label{sec:org2204013}
\begin{frame}[label={sec:orgcc287de},fragile]{Cross-validation}
 \begin{center}
...
\label{sec:orgbb221e6}
\subsection{nested cv}
\label{sec:orgf452079}
\begin{frame}[label={sec:org1521e25}]{Need for regularization}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:orgf9f9ba3}]{Regularization}
...
\end{example}
\end{frame}
\begin{frame}[label={sec:orgbf33336}]{}
\begin{columns}
...
\begin{frame}[label={sec:orgec8d81c}]{Setting hyperparameters}
...
\end{frame}
\begin{frame}[label={sec:org2b5beb2}]{Nested cross-validation}
When you hear "best", "maximum", "select", \ldots{} think "bias"
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org2ef263f}]{One split}
\begin{center}
\includegraphics[width=.9\linewidth]{figures/generated/train_eval_test/datasets.pdf}
\end{center}
\end{frame}
\begin{frame}[label={sec:orgdbdf3d7},fragile]{Nested cross-validation}
 \begin{center}
...
\begin{frame}[label={sec:orgd70efa5},fragile]{Nested cross-validation with scikit-learn}
 \begin{itemize}
...
\end{frame}
\begin{frame}[label={sec:org784a8c1},fragile]{Implementing nested CV}
...
\label{sec:orgbfd778b}
\subsection{Intro}
\label{sec:org97679e7}
\begin{frame}[label={sec:orgf96025c}]{Dimensionality reduction}
Linear regression: projection on the column space of \(X\)
\vspace{10pt}
\begin{structureenv} %% top
\begin{columns}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:orgc976aac}]{Dimensionality reduction}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org9a8b366},fragile]{Simulated data for linear regression}
 \begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgdfcd7cf}]{Model complexity: overfitting}
\begin{itemize}
...
\label{sec:org3e988b8}
\begin{frame}[label={sec:org1bde255}]{Univariate feature selection}
\begin{itemize}
...
\begin{frame}[label={sec:orgb331ee8}]{Original regression problem}
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
\hat{\y} = \X \, \hat{\bbeta}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org4c7f0c6}]{After univariate feature selection}
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
\hat{\y} = \X \, \hat{\bbeta}
...
\begin{frame}[label={sec:org76b7098}]{After univariate feature selection}
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
\hat{\y} = \X \, \hat{\bbeta}
...
\begin{frame}[label={sec:org8f3765d}]{Univariate feature selection}
...
\label{sec:org96abdbc}
\begin{frame}[label={sec:orgeb1cf74}]{Dataset transformations}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgbeabcbe},fragile]{scikit-learn "transformer API": \texttt{fit; transform}}
 \begin{minted}[]{python}
...
\begin{frame}[label={sec:orgd890872},fragile]{\texttt{feature\_selection.SelectKBest}}
...
\begin{frame}[label={sec:orge40898a},fragile]{Fit the transformer only on train data!}
 \begin{minted}[]{python}
transformer = SelectKBest()
transformed_train = transformer.fit_transform(
    X_train, y_train)
...
\begin{frame}[label={sec:org3cc674e},fragile]{Pipelines}
...
\label{sec:orga36ce79}
\begin{frame}[label={sec:org691dab6}]{Linear decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org4bacb31}]{Linear regression: projection on the column space of \(X\)}
\begin{structureenv} %% top
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:orgc261370}]{Linear decomposition: low-rank approximation of \(\X\)}
Minimize
\begin{equation}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org9356430}]{Linear regression after dimensionality reduction}
\begin{equation}
\hat{\y} = \W \, \hat{\bbeta}
\end{equation}
\begin{center}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgdb2048e}]{Prediction for a new data point \(\x \in \R^{p}\)}
\begin{itemize}
...
\x \in \R^p \rightarrow \text{projection} \rightarrow \mathbold{w} \in \R^k \rightarrow \langle \cdot \, , \, \hat{\bbeta}\rangle \rightarrow \hat{y} \in \R
    \end{equation}
\end{frame}
\begin{frame}[label={sec:org4057f22}]{Principal Component Analysis}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org7f0024e}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:org6644633}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:orga663327}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:orgce9a4d3}]{Other decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgddc3954}]{Ridge regression and PCA}
\begin{itemize}
...
\label{sec:org7f36cd5}
\begin{frame}[label={sec:orgc657978}]{(Cross-)validation experiments are simulations}
...
\end{frame}
\begin{frame}[label={sec:org89f577f}]{(Cross-)validation experiments are simulations}
The validation experiments must simulate what will happen when deploying the trained model in production -- when starting to use it in real life.
...
\end{example}
\end{frame}
\begin{frame}[label={sec:orge9cd2b7}]{Split choice example: time series}
...
\end{frame}
\begin{frame}[label={sec:org4c12d08}]{Remember that CV training sets overlap}
\begin{center}
...
\begin{frame}[label={sec:org0595241},fragile]{Some pitfalls with cross-validation}
 \small
