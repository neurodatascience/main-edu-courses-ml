...
\begin{frame}[label={sec:org3d7be5a}]{Recap of part 1}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgdf8d6eb}]{Notation \& vocabulary}
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org67f0021}]{How to set parameters: Empirical Risk Minimization}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgb673fe3}]{How to set parameters: Empirical Risk Minimization}
...
\begin{frame}[label={sec:org6bb24bd}]{Evaluating a model}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:orgb1b747e}]{Training error is a biased estimator of the risk}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgb53b261}]{Training error is a biased estimator of the risk}
\begin{itemize}
\item 30 different models (eg 30 possible values for \(\hat{\beta}_0, \hat{\bbeta}\))
\item All have a risk (expected accuracy) of 0.5
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org3123f01}]{Training error is a biased estimator of the risk}
...
\begin{frame}[label={sec:org95093be}]{Training error is a biased estimator of the risk}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org9ae4593}]{Training error is a biased estimator of the risk}
...
\begin{frame}[label={sec:orga4f3d73}]{Training error is a biased estimator of the risk}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org4924b63}]{Training error is a biased estimator of the risk}
\begin{itemize}
...
\begin{frame}[label={sec:orgc4e7452}]{Estimating prediction performance}
...
\label{sec:orgf33b892}
\begin{frame}[label={sec:org4d85803},fragile]{scikit-learn "estimator API": \texttt{fit; predict}}
 \begin{minted}[]{python}
estimator = Ridge()
...
\end{frame}
\begin{frame}[label={sec:orgdbecfa6},fragile]{Evaluating performance with \texttt{sklearn.metrics}}
 \begin{minted}[]{python}
estimator = Ridge()
estimator.fit(X_train, y_train)
predictions = estimator.predict(X_test)
...
\begin{frame}[label={sec:org31c3760},fragile]{Some possible metrics for regression}
...
\subsection{cv}
\label{sec:orgfe6c198}
\begin{frame}[label={sec:orgadf0d85},fragile]{Cross-validation}
 \begin{center}
...
\label{sec:org6032615}
\subsection{nested cv}
\label{sec:org531c3b8}
\begin{frame}[label={sec:org1b8af70}]{Need for regularization}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:orga079ad6}]{Regularization}
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org4a19d5c}]{}
\begin{columns}
...
\begin{frame}[label={sec:orgd7ac0a4}]{Setting hyperparameters}
...
\end{frame}
\begin{frame}[label={sec:org747f557}]{Nested cross-validation}
When you hear "best", "maximum", "select", \ldots{} think "bias"
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org64e50e6}]{One split}
\begin{center}
\includegraphics[width=.9\linewidth]{figures/generated/train_eval_test/datasets.pdf}
\end{center}
\end{frame}
\begin{frame}[label={sec:org85da876},fragile]{Nested cross-validation}
 \begin{center}
...
\begin{frame}[label={sec:org3e21d6a},fragile]{Nested cross-validation with scikit-learn}
 \begin{itemize}
...
\end{frame}
\begin{frame}[label={sec:org57fb23c},fragile]{Implementing nested CV}
...
\label{sec:orgff8fca0}
\subsection{Intro}
\label{sec:org08631bd}
\begin{frame}[label={sec:orgb1cb1ac}]{Dimensionality reduction}
Linear regression: projection on the column space of \(X\)
\vspace{10pt}
\begin{structureenv} %% top
\begin{columns}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:orgddaf9fa}]{Dimensionality reduction}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org8c45053},fragile]{Simulated data for linear regression}
 \begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orga3f64eb}]{Model complexity: overfitting}
\begin{itemize}
...
\label{sec:org5aac158}
\begin{frame}[label={sec:org0ab5e2c}]{Univariate feature selection}
\begin{itemize}
...
\begin{frame}[label={sec:org8adbb03}]{Original regression problem}
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
\hat{\y} = \X \, \hat{\bbeta}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:orgfc29ded}]{After univariate feature selection}
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
\hat{\y} = \X \, \hat{\bbeta}
...
\begin{frame}[label={sec:org2a8ea6c}]{After univariate feature selection}
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
\hat{\y} = \X \, \hat{\bbeta}
...
\begin{frame}[label={sec:org9b2ad98}]{Univariate feature selection}
...
\label{sec:org7109c1c}
\begin{frame}[label={sec:org82bf77c}]{Dataset transformations}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org138a5d5},fragile]{scikit-learn "transformer API": \texttt{fit; transform}}
 \begin{minted}[]{python}
...
\begin{frame}[label={sec:orgd28c711},fragile]{\texttt{feature\_selection.SelectKBest}}
...
\begin{frame}[label={sec:org7399868},fragile]{Fit the transformer only on train data!}
 \begin{minted}[]{python}
transformer = SelectKBest()
transformed_train = transformer.fit_transform(
    X_train, y_train)
...
\begin{frame}[label={sec:orgfdb8098},fragile]{Pipelines}
...
\label{sec:org302dc90}
\begin{frame}[label={sec:orge1eea65}]{Linear decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orga3d5c8f}]{Linear regression: projection on the column space of \(X\)}
\begin{structureenv} %% top
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org79ecd3b}]{Linear decomposition: low-rank approximation of \(\X\)}
Minimize
\begin{equation}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org28c6f76}]{Linear regression after dimensionality reduction}
\begin{equation}
\hat{\y} = \W \, \hat{\bbeta}
\end{equation}
\begin{center}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgb442ca8}]{Prediction for a new data point \(\x \in \R^{p}\)}
\begin{itemize}
...
\x \in \R^p \rightarrow \text{projection} \rightarrow \mathbold{w} \in \R^k \rightarrow \langle \cdot \, , \, \hat{\bbeta}\rangle \rightarrow \hat{y} \in \R
    \end{equation}
\end{frame}
\begin{frame}[label={sec:orgafe30da}]{Principal Component Analysis}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org87018de}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:org469a179}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:org9a334ac}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:org469b5b2}]{Other decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgcb3a993}]{Ridge regression and PCA}
\begin{itemize}
...
\label{sec:org621e658}
\begin{frame}[label={sec:org6ea5352}]{(Cross-)validation experiments are simulations}
...
\end{frame}
\begin{frame}[label={sec:org984fd72}]{(Cross-)validation experiments are simulations}
The validation experiments must simulate what will happen when deploying the trained model in production -- when starting to use it in real life.
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org00cf821}]{Split choice example: time series}
...
\end{frame}
\begin{frame}[label={sec:org9d6d85f}]{Remember that CV training sets overlap}
\begin{center}
...
\begin{frame}[label={sec:orgeab76bf},fragile]{Some pitfalls with cross-validation}
 \small
